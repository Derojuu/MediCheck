    ...
    for (let i = 0; i < Math.min(3, batches.length); i++) {
      ...
      await prisma.ownershipTransfer.create({
        data: {
          id: `transfer-${i + 1}`,
          batchId: batch.id,
          fromOrgId: manufacturerOrg?.id || organizations[0].id,
          toOrgId: recipient.id,
    ...
    for (let i = 0; i < Math.min(2, batches.length); i++) {
      ...
      await prisma.scanHistory.create({
        data: {
          id: `scan-${i + 1}`,
          batchId: batch.id,
          scanLocation: batch.currentLocation,
          scanDate: new Date(),
          scanResult: 'GENUINE',
...
async function seedWithLocalData() {
  ...
  for (const batch of batches) {
    const { organizationId, ...batchData } = batch;
    await prisma.medicationBatch.upsert({
      where: { id: batch.id },
      update: {},
      create: {
        ...batchData,
        organization: {
          connect: { id: organizationId || 'org-1' }
        }
      },
    })
  }
